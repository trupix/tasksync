...
import { installForClient, checkClientStatus, getClientForProvider } from "../mcp/install";
...
// ---------------------------------------------------------------------------
// Markdown → HTML (powers the Help modal from README.md)
// ---------------------------------------------------------------------------
...
function markdownToHtml(md: string): string {
  // Normalize all line endings to LF before parsing
  ...
  function inline(s: string): string {
    const codes: string[] = [];
    s = s.replace(/`([^`]+)`/g, (_: string, m: string) => { codes.push(m); return `\x01${codes.length - 1}\x02`; });
    s = s.replace(/\*\*([^*\n]+)\*\*/g, '<strong>$1</strong>');
    s = s.replace(/\*([^*\n]+)\*/g, '<em>$1</em>');
  ...
  function makeId(rawText: string): string {
    const base = rawText.replace(/`[^`]*`/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'section';
  ...
  function flushTable(): void {
    ...
    if (body.length) html += '<tbody>' + body.map(r => '<tr>' + parseRow(r).map(c => `<td>${inline(escMd(c))}</td>`).join('') + '</tr>').join('') + '</tbody>';
    out.push(html + '</table>');
  }
  ...
  for (const line of lines) {
    if (line.startsWith('```')) {
      ...
      } else { flushList(); flushTable(); inCode = true; codeLang = line.slice(3).trim() || 'text'; }
      continue;
    }
    if (inCode) { codeAcc.push(line); continue; }

    if (line.trim().startsWith('|') && line.includes('|', 2)) {
      flushList(); tableAcc.push(line); inTable = true; continue;
    }
    ...
    if (hm) {
      flushList();
      const level = hm[1].length, rawText = hm[2].trim();
      out.push(`<h${level} id="${makeId(rawText)}">${inline(escMd(rawText))}</h${level}>`);
      continue;
    }
    if (/^[-*_]{3,}$/.test(line.trim())) { flushList(); out.push('<hr>'); continue; }
    if (line.startsWith('> ')) { flushList(); out.push(`<blockquote><p>${inline(escMd(line.slice(2)))}</p></blockquote>`); continue; }

    const ulm = line.match(/^(\s*)[-*]\s+(.+)$/);
    if (ulm) {
      flushTable();
      if (!inList || listTag !== 'ul') { flushList(); out.push('<ul>'); inList = true; listTag = 'ul'; }
      out.push(`<li>${inline(escMd(ulm[2]))}</li>`); continue;
    }
    const olm = line.match(/^\d+\.\s+(.+)$/);
    if (olm) {
      flushTable();
      if (!inList || listTag !== 'ol') { flushList(); out.push('<ol>'); inList = true; listTag = 'ol'; }
      out.push(`<li>${inline(escMd(olm[1]))}</li>`); continue;
    }
...
let DOCS_HTML = '<p style="color:#71717a;">Documentation not available. See <a href="https://github.com/trupix/tasksync" target="_blank">README.md</a> on GitHub.</p>';
try {
  DOCS_HTML = markdownToHtml(fs.readFileSync(path.join(__dirname, '../../README.md'), 'utf8'));
} catch { /* README.md not found at runtime */ }
...
export async function startDashboard(port: number, openBrowser: boolean) {
  ...
  app.post("/api/mcp/install", requireAuth, (req, res) => {
    try {
